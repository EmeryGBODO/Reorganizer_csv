...
import { Download, UploadCloud, Server, Filter, X, ChevronLeft, ChevronRight, CheckCircle, RotateCcw, Import } from 'lucide-react';
...
const Stepper = ({ currentStep }: { currentStep: Step }) => {
  ...
  return (
    ...
    <ol role="list" className="flex gap-8 items-center">
      {steps.map((step, stepIdx) => (
        <li key={step.title} className="relative flex flex-col items-center">
          {/* Ligne de progression */}
          {stepIdx < steps.length - 1 && (
            <div
              className={`absolute top-1/3 -translate-y-1/3 left-16 w-[calc(100%+1rem)] h-0.5 ${
          ...
          {/* Indicateur d'étape */}
          {stepIdx < currentStepIndex ? (
            <div className="relative flex h-10 w-10 items-center justify-center bg-blue-600 rounded-full hover:bg-blue-700">
              <CheckCircle className="h-5 w-5 text-white" aria-hidden="true" />
            </div>
          ) : stepIdx === currentStepIndex ? (
            <div
              className="relative flex h-10 w-10 items-center justify-center bg-white border-4 border-blue-600 rounded-full"
              aria-current="step"
            >
              <span
            ...
            <div className="group relative flex h-10 w-10 items-center justify-center bg-white border-2 border-gray-300 rounded-full hover:border-gray-400">
              <span
                className="h-3 w-3 bg-transparent rounded-full group-hover:bg-gray-300"
                aria-hidden="true"
              />
            </div>
          )}
          ...
          {/* Titre de l'étape */}
          <span className="mt-2 text-center text-sm text-gray-700">
            {step.title}
          </span>
        </li>
      ))}
...
const EndUserPage: React.FC = () => {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 'select_source':
        return (
          <div className="p-8">
            <div className='flex justify-between items-center px-4 py-2 mb-'>
          ...
          <div className="p-6 border w-1/2 mx-auto border-gray-200 rounded-lg flex flex-col items-center justify-center text-center hover:shadow-lg transition-shadow space-y-4">
  ...
  <div className="space-x-2">
    ...
    <button onClick={()=>navigate("/import")} className="inline-flex items-center px-3 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
          <Import className="h-4 w-4 mr-1" /> Importer csv
    </button>
      <button
